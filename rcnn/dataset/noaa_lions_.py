...
DATA_PATH = "/home/aakuzin/dataset"

class noaa_lions(IMDB):
    def __init__(self, image_set, root_path, devkit_path):
        """
        fill basic information to initialize imdb
        :param image_set: noaa_lions
        :param root_path: 'selective_search_data' and 'cache'
        :param devkit_path: data and results
        :return: imdb object
        """
        super(noaa_lions, self).__init__('noaa_lions', image_set, root_path, devkit_path)  # set self.name

        self.simple_labels = pd.read_csv(os.path.join(DATA_PATH, "correct_coordinates.csv"))

        self.root_path = root_path
        self.devkit_path = devkit_path

        self.data_path = DATA_PATH #os.path.join('d:/patches_{}'.format(image_set))

        self.classes = ['__background__',  # always index 0
                        'adult_males','subadult_males','adult_females','juveniles','pups']

        self.num_classes = len(self.classes)
        self.image_set_index = self.simple_labels['filename'].str.replace('.jpg', '').unique()
        ...
        self.deltas = {'pups': 12, 'juveniles': 40, 'subadult_males': 30, 'adult_females': 25, 'adult_males': 30}
    ...
    def image_path_from_index(self, index):
        ...
        image_file = os.path.join(self.data_path,  'Train', index + '.jpg')
    ...
    def load_pascal_annotation(self, index):
        ...
        for ix in df_c.index:
            if self.image_set.find('test') != -1 or self.image_set == 'dstl_train_2000':
                ...
                x2 = 0
                y2 = 0
                klass = '__background__'
            else:
                xc = df_c.loc[ix, 'x_coord']
                yc = df_c.loc[ix, 'y_coord']
                klass = df_c.loc[ix, 'category']

                x1 = float(xc - self.deltas[klass])
                x2 = float(xc + self.deltas[klass])
                y1 = float(yc - self.deltas[klass])
                y2 = float(yc + self.deltas[klass])
