"""
General purpose database with Pascal VOC annotations
...
from general_eval import voc_eval
from ds_utils import unique_boxes, filter_small_boxes


class General(IMDB):
    def __init__(self, image_set, root_path, devkit_path, classes):
        """
        fill basic information to initialize imdb
        :param image_set: train, val, etc
        :param root_path: 'selective_search_data' and 'cache'
        :param devkit_path: data and results
        :param store: whether to store all dataset im memory
        :return: imdb object
        """
        super(General, self).__init__("general", image_set, root_path, devkit_path)  # set self.name
    ...
    def load_pascal_annotation(self, index):
        ...
        filename = os.path.join(self.data_path, 'annotations', index.rsplit(".", 1)[0] + '.xml')
        ...
        for ix, obj in enumerate(objs):
            ...
            x1 = np.clip(x1, a_min=0, a_max=roi_rec['width']-1)
            y1 = float(bbox.find('ymin').text) - 1
            y1 = np.clip(y1, a_min=0, a_max=roi_rec['height']-1)
